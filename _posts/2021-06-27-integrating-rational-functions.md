...
**Definition 3.** A **unit** in a (unital) ring (with unity $$1$$) is an element $$u \in R$$ such that there exists $$v \in R$$ with $$uv = vu = 1$$; then $$v = u^{-1}$$ is a **(multiplicative) inverse** of $$u$$. The set of units in $$R$$ is denoted $$R^*$$; it is a group (it satisfies precisely (A2)–(A4) but for multiplication instead), called the **unit group**.
