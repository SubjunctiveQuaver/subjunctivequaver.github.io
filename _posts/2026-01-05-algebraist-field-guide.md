...
- Aren't we essentially just quotienting out null sequences from the Cauchy sequences?
...
- Why don't more people talk about the beautiful polynomial quotient construction of complex numbers? (Genuine question!)
...
With that in mind, we'll walk through some standard ways of building the main number systems -- from $\mathbb{Z}$ up to $\mathbb{O}$ -- with a distinctive algebraic flavour, noting what each construction buys you and what it costs. This post benefited from conversations with an AI assistant who is very patient with algebraists.
...
Downward-closed subsets of $\mathbb{Q}$ with no greatest element.
...
#### (4A) Polynomial quotient

Consider the polynomial ring $\mathbb{R}[x]$.

Form the quotient by the ideal generated by $x^2 + 1$, i.e., $\mathbb{C} \cong \mathbb{R}[x]/(x^2 + 1)$.
...
**Intuition:** By forcing $x^2 + 1 = 0$, we create a new element $i$ with $i^2 = -1$, just like in high school.

This algebraic construction adds a root of the polynomial $x^2 + 1$ to $\mathbb{R}$, producing the smallest field extension where this polynomial factors. From this relation, the other algebraic properties of $i$ and $\mathbb{C}$ follow naturally.
